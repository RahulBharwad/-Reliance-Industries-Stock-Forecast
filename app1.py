...
# convert an array of values into a dataset matrix
 ...
 #   symbols = 'RELIANCE.NS'
#ticker = st.sidebar.select_slider("Choose which information you want from" , symbols)

    period=('Complete','Last 7 days','Last 1 Month','Last 6 Months','Last year')
...
#st.sidebar.write(data[['Open','High','Low','Close','Adj Close','Volume']][data['Date']== selected_date])

#st.sidebar.json(data[['Open','High','Low','Close','Adj Close','Volume']][data['Date']== selected_date].iloc[0].to_dict())
    
for column, value in data[['Open','High','Low','Close','Adj Close','Volume']][data['Date']== selected_date].iteritems():
    st.sidebar.text(f"{column}: {value.values[0]}")